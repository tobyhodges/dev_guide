...
`roxygen2` does not currently completely/sufficiently support [R6](https://github.com/r-lib/R6) classes. In the meantime, here are some best practices for documenting R6 classes, starting with a brief complete example, then breaking down the pieces.
...
#' @export
#' @section Usage:
#' \preformatted{Foo$new(x = 1)}
#' @details
#' **Methods**
#'   \describe{
#'     \item{`bar(y, z)`}{
#'       Add y and z, and divide by x
#'       \tabular{ll}{
#'        y  \tab  an integer or numeric \cr
#'        z  \tab  an integer or numeric \cr
#'       }
#'     }
#'   }
#' @format NULL
...
Instead of using `@usage`, create a section for usage, like:

```r
#' @section Usage:
#' \preformatted{p <- Foo$new(x = 0)}
```

Set `@format` to not be used because it creates something not very useful:

```r
#' @format NULL
```

Create a section, could be under `@details` or in `@section Details:` or similar, with information on each public method in your R6 class.
...
#'       Make a GET request
#'       \tabular{ll}{
#'        arg1  \tab  description \cr
#'        arg2  \tab  description \cr
#'       }
#'     }
#'   }
```

There's no one size fits all approach for documenting R6. You can find
additional suggestions in the [mlr3 roxygen guide][mlr3guide], among
other places. Some R6 classes are small and can be documented easily in
a single manual page, whereas in other cases one may want separate manual
pages for each method, or groups of methods.
...
[mlr3guide]: https://github.com/mlr-org/mlr3/wiki/Roxygen-Guide
